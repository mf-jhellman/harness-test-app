...
func main() {
	app, err := newrelic.NewApplication(
		newrelic.ConfigAppName(os.Getenv("NR_APP_NAME")),
	...
	mux.HandleFunc(newrelic.WrapHandleFunc(app, "/", httpLog(stdoutW, withAppHeaders(httpEcho(*textFlag)))))

	// Health endpoint
	mux.HandleFunc(newrelic.WrapHandleFunc(app, "/health", withAppHeaders(httpHealth())))
